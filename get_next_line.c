...
/*   Updated: 2025/11/02 10:21:51 by abahoumi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buffer[(unsigned long)BUFFER_SIZE + 1];
	char		*line;
	char		*storage;
	...
	if (!storage)
		return (NULL);
	if (index_of(storage, '\n') == -1)
		storage = read_file(fd, buffer, storage);
	if (!storage)
		return (NULL);
	line = extract_line(storage);
	update_buffer(buffer, storage);
	return (line);
}
